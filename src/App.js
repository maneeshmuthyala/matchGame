...
class App extends Component {
  ...
  set = id => {
    ...
    if (id === currentImage.thumbnailUrl) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        currentImage: imagesList[randomIndex],
      }))
    } else {
  ...
  playAgain = () => {
    clearInterval(this.timeInterval)
    this.setState(initialState, this.onStartTimer)
  }
  ...
  gameBody = () => {
    const {currentImage, score, timeElapsedInSeconds} = this.state
          ...
          <ul>
            {tabsList.map(each => (
              <li key={each.tabId}>
                <button
                  className="tab"
                  onClick={() => this.setActiveTabId(each.tabId)}
                  type="button"
                >
                  {each.displayText}
                </button>
              ...
              <li key={each.id}>
                <button type="button">
                  <img
                    ...
                    className="image"
                    alt="match"
                    onClick={() => this.set(each.thumbnailUrl)}
                  />
                </button>
              </li>
            ))}
  ...
  render() {
    const {isTimerRunning, score, timeElapsedInSeconds} = this.state
    return (
      <>
          ...
          <nav className="navbar">
            <ul>
              ...
              <div className="time-cont">
                ...
                <li>
                  <p className="time">{timeElapsedInSeconds} sec</p>
                </li>
              </div>
            </ul>
          </nav>
          {isTimerRunning ? this.gameBody() : this.scoreCard()}
        </div>
      </>
    )
  }
...
class App extends Component {
  ...
  gameBody = () => {
    ...
    return (
      <>
        <div className="thumb">
          <img src={currentImage.imageUrl} className="th" alt="match" />
        </div>
        <div className="sub">
          <ul>
            {tabsList.map(each => (
          ...
          <ul>
            {filteredApps.map(each => (
              <li key={each.id}>
                <button
                  type="button"
                  onClick={() => this.handleThumbnailClick(each.id)}
                >
                  <img
